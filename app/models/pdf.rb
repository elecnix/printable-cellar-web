  ...
  def self.generate(filename, wines)
    ...
    Prawn::Document.generate(filename) {
      font_families.update(
          "Museo_Slab" => {
              :normal => Rails.root.join('assets/fonts/Museo-Slab-W01-100.ttf'),
              :bold => Rails.root.join('assets/fonts/Museo_Slab_500.ttf')
          }
      )
      dash(3, :space => 2)
      stroke_color "aaaaaa"
              ...
              bounding_box([label_width * col, (row_count * label_height) - row * label_height], :width => label_width, :height => label_height) {
                ...
                font("Museo_Slab", :size => 9) {
                  bounding_box([margin, label_height - margin], :width => box_width, :height => label_height - 2 * margin) {
                    ...
                    line_no += 0.4
                    font("Museo_Slab", :style => :bold, :size => 10) {
                ...
                rotate(90, :origin => [0, 0]) {
                  float {
                    text_box "V2.1.0  © Nicolas Marchildon 2019,  © Société des alcools du Québec, Montréal, 2007, 2010",
